...
/* Prvi monom je specijalan jer se ispred njega ne vrsi
   eksplicitan ispis znaka. 
   Na primer, za polinom x + 3*x^2, prvi monom je x.
   Svakom sledecem monomu (u ovom slucaju samo 3*x^2)
   u ispisu prethodi znak (+ ili -). 
   Funkcija ispisuje prvi monom. */
...
void ispis(const Polinom * p)
{
  ...
  /* Vrsi se ispis prvog monoma. Posto je moguce da prvi monom
...
void integral(const Polinom * p, Polinom * integ)
{
  int i;
...
int main()
{
  ...
  Polinom polinomi[MAKS_POLINOMA], integ;
  int n, i;
