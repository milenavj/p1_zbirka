...
int main()
{
  ...
  /* Ciklicni ispis elemenata matrice: 
     ...
     4. zatim na gore (2,0) (1,0)
     (ovde se staje jer je (0,0) vec ispisano) i prelazi se opet 
     na levo. Koraci 1-4 se ponavljaju dok god se ne ispisu svi
     elementi. Ideja je da kada se ispisu elementi
     prve vrste (kada se ide sa leva na desno), da se pomeri
     "gornja granica ispisa" za 1, kako bi se naznacilo da je taj
     red vec ispisan. Slicno, kada se vrsi ispis odozgo na dole,
     uspesno je ispisana jedna kolona pa je potrebno pomeriti
     ...
     na gore, pomera se leva granica ispisa za jedan u desno. */
  gornja_granica = 0;
  donja_granica = m - 1;
  leva_granica = 0;
  desna_granica = n - 1;
  ...
  for (brojac = 0; brojac < m * n; brojac++) {
    ...
    case 2:
      ...
      /* Ako je pravac = 3, trenutni smer ispisa je sa desna na
         ...
         jedno mesto na gore. A ako nije, samo se prelazi na
         narednu kolonu. */
    case 3:
      if (j == leva_granica) 
      {
