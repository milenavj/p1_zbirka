...
int podniska(char s[], char t[])
{
  ...
  for (i = 0; s[i] != '\0'; i++) {
    ...
    /* Ako je unutrasnja petlja dosla do kraja niske t, to 
       znaci da su se svi karakteri iz t poklopili sa karakterima
       iz s i t je podniska od s. */
    if (t[j] == '\0')
      return 1;
  }
...
int main()
{
  /* Deklaracija niski s i t. */
  char s[MAKS_NISKA], t[MAKS_NISKA];

  /* Ucitavaju se niske s i t. */
  ...
  /* II nacin: 
     Funkcija strstr(t, s) proverava da li je t podniska od s
     i kao povratnu vrednost vraca adresu prvog pojavljivanja t u s
     ili NULL ukoliko se t ne pojavljuje u s. 
     Deklaracija ove funkcije se nalazi u zaglavlju string.h
