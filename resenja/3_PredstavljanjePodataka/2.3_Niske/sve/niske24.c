...
char* strstr_klon(char s[], char t[])
{
  ...
  for (i = 0; s[i] != '\0'; i++) {
    ...
    /* Ako je unutrasnja petlja dosla do kraja niske t, to 
       ...
       Kao povratna vrednost se vraca mesto gde t pocinje u s. */
    if (t[j] == '\0')
      return &s[i];
  }
...
int main()
{
  ...
  /* Ucitavanje linija i ispis rednih brojeva linija
     koje sadrze rec "program". */
  for(i=1; i<=5; i++)
  {
  ...
  /* Ako indikator bar_jedna i dalje ima vrednost 0, znaci da
     nije uneta nijedna linija koja sadrzi rec "program". */
  if(!bar_jedna)
  {
