...
int main()
{
  ...
  /* Promenljive ulaz i izlaz predstavljaju pokazivace na ugradjenu
     strukturu FILE. Unutar ove strukture nalaze se polja
     neophodna za rad sa datotekama. */
  FILE *ulaz, *izlaz;
  ...
  /* Funkcija fopen sluzi da otvori datoteku. Prvi argument je
     ...
     citanje. 
     Ukoliko datoteka ne postoji, funkcija fopen kao povratnu 
     vrednost vraca NULL. */
  ulaz = fopen("ulaz.txt", "r");
  if (ulaz == NULL) 
  {
    /* Funkcija fprintf vrsi ispis u fajl.
       Funkcionise isto kao i funkcija printf - razlika je sto se
       kao prvi argument prosledjuje fajl u koji se ispisuje izlaz.
