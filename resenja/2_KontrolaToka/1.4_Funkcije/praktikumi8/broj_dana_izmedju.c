...
/* Funkcija proverava da li je datum ispravan. Ako je datum  ispravan funkcija vraca 1, inace vraca 0. */
int ispravan(int dan, int mesec, int godina)
{
   ...
   /* Ako mesec nije u opsegu od 1 do 12, datum nije ispravan. */
	...
	/* Ako je dan manji od 1 ili veci od broja dana u datom mesecu, datum nije ispravan. */
...
int od_nove_godine(int dan, int mesec, int godina)
{
	...
	/* Za sve mesece pre datog datuma dodaje se broj dana za dati mesec. */
	for(i=1; i<mesec; i++)
		suma_dana += broj_dana(mesec, godina);

	/* Na kraju se dodaje koliko je dana proteklo u datom mesecu, a to je zadato sa promenljivom dan. */
...
int do_kraja_godine(int dan, int mesec, int godina)
{
	...
	/* Za sve mesece posle datog datuma dodaje se broj dana za dati mesec. */
...
/* Funkcija vraca 1 ako je prvi datum pre drugog datuma. U suprotnom vraca 0. */
int prethodi(int dan1, int mesec1, int godina1, int dan2, int mesec2, int godina2)
{
	if (godina1 < godina2)
		return 1;
...
int broj_dana_izmedju(int dan1, int mesec1, int godina1, int dan2, int mesec2, int godina2)
{
	...
	/* Provera koji od datuma je ranije. Da bi osigurali da manji datum bude onaj uz koji ide broj 1, datumi se razmenjuju. Obratiti paznju da se u okviru ove if naredbe vise puta javlja jedan te isti deo koda i da bi bilo lepo to zapisati u okviru funkcije. Ipak, u ovom trenutku to nije moguce jer je potrebno koristiti pokazivace o kojima ce biti reci kasnije. Razmisliti kako bi se ovo resenje moglo popraviti kada se budu presli pokazivaci. */
   ...
   if (godina1 != godina2)
	{
		...
		/* Za sve godine koje su izmedju dve date godine dodaje se broj dana u tim godinama. */
	...
	else if (mesec1 != mesec2)
	{
		...
		/* Dodaje se broj dana za svaki mesec koji je izmedju dva data meseca. Kako su godina1 i godina2 jednake svejedno je koja od ove dve promenljive se koristi u pozivu funkcije. */
...
int main()
{
   ...
   printf("Unesite drugi datum:");
	...
	/* Provera ispravnosti. Ako datum nije ispravan prekida se rad programa. */
	if (!ispravan(dan1, mesec1, godina1) || !ispravan(dan2, mesec2, godina2))
	{
   ...
   /* Poziva se funkcija za ispis sledeceg dana. */
	printf("Broj dana izmedju dva datuma je:%d\n", broj_dana_izmedju(dan1, mesec1, godina1, dan2, mesec2, godina2));
