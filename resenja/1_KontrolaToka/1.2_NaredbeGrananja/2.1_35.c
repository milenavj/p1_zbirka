...
int main()
{
   ...
   switch (c)   
   {
       ...
       case 'h':
          /*
             Ukoliko se tacke A(x1,y1) i B(x2,y2) ne nalaze 
             na pravoj koja je paralelna x osi, izracunamo k,n 
             za pravu odredjenu tackama A(x1,y1) i B(x2,y2)
          */
          if ((x1-x2)!=0) 
          {
             k = (y1-y2)/(x1-x2); 
             n = y1-k*x1;
             /*
                Proverimo da li tacke C(x3,y3) i D(x4,y4) nalaze 
                na toj pravoj 
             */
          ...
          /*
             Ukoliko se A i B nalaze na pravoj koja je paralelna 
             x osi, proverimo da li tacke C(x3,y3) i D(x4,y4) 
             nalaze na toj pravoj
          */ 
          else 
             if (x3==x1 && x4==x1) 
                printf ("Tacke su kolinearne, pripadaju pravoj ",
