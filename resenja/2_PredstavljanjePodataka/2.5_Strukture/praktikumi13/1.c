...
/* Struktura koja opisuje kompleksni broj obuhvata polje za realni i polje za imaginarni deo broja. */
...
int main() {
  ...
  c = saberi(&a, &b);
  printf("Zbir: %.2f%c%.2f*i\n", c.re, c.im > 0 ? '+' : ' ', c.im); /* Ukoliko je imaginarni deo negativan, 
  ...
  if(b.re != 0 || b.im != 0) {
    ...
    printf("Kolicnik ne postoji.\n"); /* Ni u polju kompleksnih brojeva
