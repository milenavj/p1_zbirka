...
int main() {
    ...
    for(i=0; i<n; i++) {
        scanf("%d%lf%s", &studenti[i].indeks, &studenti[i].poeni, studenti[i].ucionica);
    }
    ...
    /* Odredjivanje broja studenata koji su polagali ispit u zadatoj ucionici i polozili ga. */
    broj_studenata = 0;
    for(i=0; i<n; i++) {
        if(!strcmp(ucionica, studenti[i].ucionica) && studenti[i].poeni >= 51.0) {
            broj_studenata++;
        }
    }
