...
Ako su $a = (a_1, \ldots, a_n)$ i $b = (b_1,\ldots, b_n)$ vektori dimenzije $n$, njihov skalarni proizvod je $a \cdot b = a_1\cdot b_1 + \ldots + a_n\cdot b_n$. Napisati program koji računa skalarni proizvod dva vektora. Vektori se zadaju kao celobrojni nizovi sa najviše $100$ elemenata. 
Program učitava dimenziju i elemente nizova, a na izlaz ispisuje vrednost skalarnog proizvoda.
% ili odgovarajuću poruku u slučaju greške. 
...
Napisati program koji za učitani ceo broj, ispisuje broj pojavljivanja svake od cifara u zapisu tog broja. \uputstvo{Za evidenciju broja pojavljivanja svake cifre pojedinačno, koristiti niz.} 
...
%Ovaj zadatak je isti kao zadatak kod funkcija sa labelom p1.4_12 i to nekako treba povezati 
...
%TODO koristi se while, a trebalo bi do-while kao u prethodnom zadatku, jer inace nece raditi dobro za nulu

%TODO trebalo bi to izdvojiti u funkcije: inicijalizacija brojaca, prebrojavanje, i provera jednakosti nizova

%TODO resenje kakvo je sada nije dobro jer su oni vec ucili funkcije sto znaci da treba da ih upotrebljavaju uvek kada to ima smisla, a ovde ima dupliranog koda i u skladu sa time uvodjenje funkcija je vazno

%TODO kada se ovo sve izdvoji u funkcije, treba ovaj zadatak pomeriti tamo gde pocinju zadaci koji kombinuju nizove i funkcije
...
Napisati program koji učitava karaktere sa standardnog ulaza sve do kraja ulaza i izračunava koliko se puta u unetom tekstu pojavila svaka od cifara, svako malo slovo i svako veliko slovo. Ispisati broj pojavljivanja samo za karaktere koji su se u unetom tekstu pojavili barem jednom. \uputstvo{Za evidenciju broja pojavljivanja cifara, malih i velih slova korisiti pojedinačne nizove.} 

%TODO smisliti neki lep test primer u Andjelkinom stilu, dakle da je nezan i da ima smisla

%TODO u resenju bi bilo dobro izdvojiti ispis u jednu funkciju umesto trostrukog ponavljanja istog koda

%TODO u svim resenjima gde imamo dvostruko isisivanje ili unosenje elemenata niza, bilo bi dobro da se ne radi copy-paste, vec da se to izdvoji u funkciju, toga ima na dosta mesta u resenjima. Trebalo bi da ono ucitaj i stampaj se izdvoji vec u neki od zadataka pri pocetku i da se onda posle svuda koristi, a ne da se ceka zadatak 12 za to, mnogo je kasno, onda imamo previse losih resenja. 

%TODO recimo da je ovo pravilo: ako imamo vise od jednog unosa ili ispisa, onda koristimo funkciju, inace ucitavamo i ispisujemo nizove u main-u
...
%TODO resiti --- ne mora bas sve da bude reseno!!!
...
%smisliti neke lepe test primer u Andjelkinom stilu, dakle da su nezani i da imaju smisla

\begin{maxitest}
\begin{upotreba}{1}
#\naslovInt#
#\ulaz{haHJjkL}#
#\izlaz{ a:1  b:0  c:0  d:0  e:0  f:0  g:0  h:2  i:0  j:2  k:1  l:1  m:0}#
#\izlaz{ n:0  o:0  p:0  q:0  r:0  s:0  t:0  u:0  v:0  w:0  x:0  y:0  z:0}#
\end{upotreba}
\end{maxitest}
...
#\izlaz{a:5  b:0  c:0  d:2  e:1  f:0  g:0  h:0  i:1  j:1  k:1  l:1  m:1}#
#\izlaz{n:1  o:3  p:1  q:0  r:1  s:1  t:0  u:1  v:0  w:0  x:0  y:0  z:0}#
\end{upotreba}
\end{maxitest}
...
Napisati program koji za dva učitana niza $a$ i $b$ dimenzije $n$ formira i na izlaz ispisuje niz $c$ koji se dobija naizmeničnim raspoređivanjem elemenata nizova $a$ i $b$, tj.~$c = [a_0, b_0, a_1, b_1, \ldots, a_{n-1}, b_{n-1}]$. Pretpostaviti da dimenzija učitanih nizova nije veća od 100.  
...
%TODO preformulisati zadatak tako da bude kao prethodni
 Sa standardnog ulaza se učitava ceo broj $n$ (manji od $100$) i elementi dvaju nizova $a$ i $b$ dimenzije $n$. Napisati program koji formira i ispisuje niz $c$ čiju prvu polovinu čine elementi niza $b$, a drugu polovinu elementi niza $a$. %U slučaju greške ispisati odgovarajuću poruku.
...
\begin{Exercise}[label=p.nizovi_spajanje_sortiranih] 
 Sa standardnog ulaza se učitava ceo broj $n$ manji od $100$ i elementi dvaju sortiranih neopadajuće nizova $a$ i $b$ dimenzije $n$. Napisati program koji formira i ispisuje niz $c$ koji se dobija spajanjem nizova $a$ i $b$ u treći, takođe sortiran neopadajuće, niz. %U slučaju greške ispisati odgovarajuću poruku.

%Ovo je deo merge sort algoritma a to je p2 --- cini mi se da to ipak ne treba raditi ovde
...
\emph{rupa} koje su nastale eliminacijom elemenata i ispisati na standardni izlaz. 
%U slučaju greške, ispisati odgovarajuću poruku. 
...
%Svi zadaci u ovom poglavlju su prilicno bez neke atraktivnosti i bez semantike. Ovom bi jednostavno mogla da se doda semantika. suma da bude npr ukupan broj artikala prodat u jednoj prodavnici, prosek da bude prosecna vrednost prodatih artikala, usput tu bi povratna vrednost trebala da bude float, minimum da bude najjeftiniji a maksimum najskuplji proizvod

Napisati funkcije za rad sa nizovima celih brojeva. 
\begin{enumerate}
\item Napisati funkciju \kckod{void ucitaj(int a[], int n)} koja učitava elemente niza $a$ dimenzije $n$. 
\item Napisati funkciju \kckod{void stampaj(int a[], int n)} koja štampa elemente niza $a$ dimenzije $n$.
\item Napisati funkciju \kckod{int suma(int a[], int n)} koja računa i vraća sumu elemenata niza $a$ dimenzije $n$.   
\item Napisati funkciju \kckod{int prosek(int a[], int n)} koja računa i vraća prosečnu vrednost (aritmetičku sredinu) elemenata niza $a$ dimenzije $n$.
\item Napisati funkciju \kckod{int minimum(int a[], int n)} koja izračunava i vraća minimum elemenata niza $a$ dimenzije $n$.
\item Napisati funkciju \kckod{int pozicija\_maksimuma(int a[], int n)} koja izračunava i vraća poziciju maksimalnog elementa u nizu $a$ dimenzije $n$. U slučaju više pojavljivanja maksimalnog elementa, vratiti najmanju poziciju.  
\end{enumerate}
...
%TODO delove pod d i e izdvojiti u poseban zadatak.
%TODO delovima pod a,b,c i d  dodati neku semantiku, artikli u prodavnici ili nesto mastovitije
...
%\item Napisati funkciju koja pronalazi najdužu seriju jednakih elemenata u nizu.
\end{enumerate}
Napisati i program koji testira rad napisanih funkcija za uneti broj $m$ i niz čija dimenzija nije veća od 100. %U slučaju greške ispisati odgovarajuću poruku. 
...
%TODO ovo pod a nije ista vrsta zadatka kao ovo ostalo tako da bi ovo mozda trebalo razdvojiti. Takodje, ako se razdvoji, ima smisla dati neku semantiku ovom zadatku. Na primer, u nizu su brojevi necega sto poseduju studenti, i onda je svako od njih dobio jos m toga, pa da vidimo koliko sada voni toga imaju... ili tako nesto
...
Napisati program koji transformiše uneti niz tako što kvadrira sve negativne elemente niza. Pretpostaviti da je dimenzija niza broj koji nije veći od 100. %U slučaju greške ispisati odgovarajuću poruku.
...
%TODO ovo bi znacilo resenje sabrojem 99, a to nije ono sto zelimo
Sa standardnog ulaza se učitava dimenzija niza, elementi niza i jedan ceo broj $k$. Napisati program koji štampa indekse elemenata koji su deljivi sa $k$. Pretpostaviti da je dimenzija niza broj koji nije veći od 100.  %U slučaju greške ispisati odgovarajuću poruku. 
...
Napisati program koji učitava dimenziju i elemente niza i štampa niz u kojem su najveći i najmanji element niza razmenili mesta. Ukoliko se najmanji ili najveći element više puta pojavljuju u nizu, uzeti u obzir njihova prva pojavljivanja. Pretpostaviti da je dimenzija niza broj koji nije veći od 100.  %U slučaju greške ispisati odgovarajuću poruku.
...
%TODO resiti - ne mora sve da bude resenjo!
...
elementa u nizu $a$ dimenzije $n$ koristeći samo jedan prolaz kroz niz. Funkcija kao povratnu vrednost vraća
manji od ta dva indeksa. Napisati program koji testira ovu funkciju za učitane nizove celih brojeva maksimalne dužine $100$ elemenata. %U slučaju greške ispisati odgovarajuću poruku. 
...
\begin{Exercise}[label=p.niz_karaktera_obrnuto] 
 Napisati program koji učitane karaktere (najviše njih $100$, učitavaju se sve do pojave karaktera \textit{*}) ispisuje u redosledu suprotnom od redosleda čitanja. 
...
%TODO dodati semantiku ovom zadatku
%mozda umesto 3 puta, da bude n puta gde se n isto ucitava

%TODO ovaj zadatak treba da ide nakon zadatka sa linearnom pretragom, jer je u pitanju ista tehnika resavanja problema
 Sa standardnog ulaza se unosi broj elemenata niza $a$ i njegovi elementi. Napisati program koji od datog niza formira niz $b$ u koji ulaze elementi niza $a$ koji se pojavljuju tačno tri puta. %U slučaju greške, ispisati odgovarajuću poruku.  
...
%TODO: resiti - ne mora sve da bude reseno!!!
\begin{Exercise}[label=vp.bez_resenja_1] 
Napisati funkciju \kckod{int sadrzi\_bar\_dva(int a[], int na, int b[], int nb)} koja proverava da li niz $a$ dužine $na$ sadrži barem dva broja koja se pojavljuju u nizu $b$ dužine $nb$. 
%Funkcija vraća vrednost $1$ ako je svojstvo ispunjeno ili $0$ ako nije.  
Napisati i program koji učitava redom dimenzije i elemente nizova $a$ i $b$ i ispisuje da li uneti nizovi ispunjavaju traženo svojstvo. Pretpostaviti da će uneti nizovi imati najviše $100$ elemenata. 
...
\begin{Exercise}[label=p.unija_presek_razlika] 
 Sa standardnog ulaza se, redom, učitavaju dimenzije i elementi dva niza, $a$ i $b$. Napisati program koji određuje i ispisuje njihovu uniju, presek i razliku (redosled prikaza elemenata nije bitan). Pretpostaviti da će uneti nizovi imati najviše $100$ elemenata. 
 
%TODO ako je manje od 100, to je onda 99. A mi u resenju pisemo 100. To onda znaci da bi trebalo da bude "nece imati vise od 100 elemenata" ili "manje ili jednako 100" To bi trebalo na neki nacin svuda ujednaciti i promeniti. Takodje, mozda bi trebalo praviti i nizove od 50 elemenata, cisto da ne bude svaki dimenzije 100, jer ce misliti da je to jedina postojeca dimenzija i zabravice da to citaju vec ce samo svuda pisati 100... 
...
Napisati program koji za učitani niz formira i ispisuje niz koji se dobija izbacivanjem svih neparnih elemenata niza. Zadatak rešiti na dva načina: korišćenjem pomoćnog niza i transformacijom polaznog niza. Pretpostaviti da će uneti niz imati najviše $100$ elemenata.  %U slučaju greške, ispisati odgovarajuću poruku.
...
Napisati program koji za učitani niz ispisuje niz koji se dobija izbacivanjem svih elemenata koji su prosti brojevi. Zadatak rešiti na dva načina: korišćenjem pomoćnog niza i transformacijom polaznog niza. %U slučaju greške, ispisati odgovarajuću poruku. 
Pretpostaviti da će uneti niz imati najviše $100$ elemenata. 
\napomena{Broj $1$ nije prost}. 

%TODO podesiti resenje sa time da su prosti brojevi defnisani samo za prirodne brojeve, i da 1 po definiciji nije prost broj. 
...
%TODO pretpostavke o velicini niza nisam ujednacila, ima ih od bar dve vrste. Uglavnom su na kraju zadatka i trebalo bi da to uvek bude recenica iste formulacije, izaberi koja ti se najvise svidja, i dodaj to na ona mesta koja sam ja greskom preskocila. 

\begin{Exercise}[label=p.broj_cifara] 
 Napisati funkciju \kckod{int cifre(char s[], int n)} koja izračunava broj cifara u nizu karaktera $a$ dužine $n$. Napisati program koji za karaktere koji se unose u zasebnim redovima ispisuje broj unetih cifara. Pretpostaviti da dužina niza neće biti veća od $100$. 
...
\begin{Exercise}[label=p.kvadriranje_parnih] 
 Napisati funkciju \kckod{void kvadriranje(float a[], int n)} koja kvadrira elemente realnog niza $a$ dužine $n$ koji se nalaze na parnim pozicijama. Napisati program koji tranformiše na ovaj način uneti niz. Pretpostaviti da dužina niza neće biti veća od $100$. 
...
Napisati funkciju \kckod{int blizu\_3(int a[],int n)} koja pronalazi i vraća indeks elementa niza koji je po vrednosti najbliži aritmetičkoj sredini onih elemenata niza koji su deljivi brojem tri. Napisati program koji testira rad funkcije. Pretpostaviti da dužina niza neće biti veća od $100$. 
...
%komentar na ovu temu sam stavila uz naredni zadatak, vazi i ovde
...
%Ne svidja mi se da je zadatak kompletno preuzet. Da li je zadatak tamo resen? Ako jeste, onda je to skroz vezveze. Ali, zadatak jeste vazan, a glupo je da je skroz preuzet. Da li moze nekako da se preformulise da bude drugaciji? Za pocetak da bude pet razlicitih zadataka, mozda im dati neku semantiku, npr u nizu se nalaze brojevi koji oznacavaju nesto, pa onda napraviit niz koji ima nekakvu osobinu... npr broj prodatih artikala u prodavnici za jedan mesec od 30 dana pa naci koliko najduze je iz dana u dan broj prodatih artikala rastao (duzina najveceg neopadajuceg podniza)...
...
Napisati funkciju koja iz zadatog niza izbacuje sve elemente koji su deljivi svojim indeksom. Niz reorganizovati tako da nema \emph{rupa} koje su nastale izbacivanjem elemenata. Povratna vrednost funkcije je nova dimenzija niza. Napisati program koji za učitni niz (dimenzije manje od $100$) ispisuje niz dobijen nakon poziva funkcije. %U slučaju greške ispisati odgovarajuću poruku. 
\napomena{Element na nultoj poziciji niza zadržati jer nije dozvoljeno deljenje nulom.}

%ovde nije jasno da li treba i da nakon pomeranja vazi isto to pravilo da element nije deljiv sa svojim indeksom? npr, 0 1 4 6 --- izbacimo broj 4 jer je deljiv sa dva, i onda se dobija niz 0 1 6 --- da li onda treba da izbacimo i 6 jer je deljiv sa dva?

%TODO dodati da svuda bude minimalno dva test primera
...
\begin{enumerate}
\item  Napisati funkciju \kckod{void brojanje(int a[], int b[], int n)} koja na osnovu celobrojnog niza $a$ dimenzije $n$ formira niz $b$ tako što $i$-ti element niza $b$ odgovara broju pojavljivanja vrednosti $i$ u nizu $a$. 
\item Napisati funkciju \kckod{int permutacija(int a[], int n)} koja proverava da li je zadati niz permutacija. Funkcija vraća vrednost $1$ ako je svojstvo ispunjeno, odnosno $0$ ako nije. \uputstvo{Koristiti funkciju $brojanje$ iz tačke (a).} 
\end{enumerate}
