...
\begin{Exercise}[label=NIS_02] 
 Napisati funkciju \kckod{void ubaci\_zvezdice(char s[])} koja menja nisku $s$ tako što u njoj svaki drugi karakter zameni zvezdicom. 
...
\begin{enumerate}
\item \kckod{int poredjenje(char s1[], char s2[])} koja vraća jedinicu ako su $s_1$ i $s_2$ jednake niske, a nulu inace.
...
Napisati program koji za učitanu nisku $s$ i karakter $c$ ispituje da li se $c$ pojavljuje u niski $s$. 
Ako se pojavljuje, program treba da ispiše indeks prvog pojavljivanja karaktera $c$ u niski $s$, a u suprotnom -1. 
...
\begin{Exercise}[label=NIS_07] 
 ...
 Napisati  program koji učitava dve niske maksimalne dužine 10 karaktera i testira rad napisane funkcije.
...
\begin{Exercise}[label=NIS_11] 
   Napisati program koji učitava nisku $s$ i formira nisku $t$
   trostrukim nadovezivanjem niske $s$. Možemo pretpostaviti da niska $s$ sadrži najviše 30 karaktera. 
...
U slucaju neispravnog unosa, ispisati odgovarajuću poruku o grešci.
...
\napomena{Linija može da sadrži blanko znakove, ali ne sadrži znak za novi red ili \kckod{EOF}.}
...
\begin{enumerate}
\item \kckod{int procitaj\_recenicu(char s[], int n)} koja učitava rečenicu i smešta je u nisku $s$. 
...
Napisati program koji testira napisanu funkciju tako što učitava pet linija i ispisuje sve 
redne brojeve linija koje sadr\v ze nisku $program$. Ukoliko ne postoji linija sa niskom $program$, 
...
Napisati program koji ucitava broj $n$ i osnove $o_1$ i $o_2$ i ispisuje dekadnu vrednost broja $n$ u osnovi $o_1$, kao i
