...
Napisati funkciju koja uređuje svoja dva celobrojna argumenta tako da se
...
\begin{Exercise}[label=POK_02] 
  Napisati funkciju koja datu boju u \textit{rgb} formatu konvertuje u boju u \textit{cmy} formatu po sledećim formulama:\\
  $$c = 1 - r / 255$$
  $$m = 1 - g / 255$$
  $$y = 1 - b / 255$$
...
\begin{Exercise}[label=POK_06] 
 Napisati funkciju \kckod{void interpunkcija(int* broj\_tacaka, int* broj\_zareza)} koja 
 sa ulaza učitava karakter po karakter sve do kraja ulaza i prebrojava sva pojavljivanja
...
\begin{Exercise}[label=POK_08] 
 Napisati funckiju \kckod{void min\_max(float a[], int n, float* najmanji, float* najveci)} koja 
 izračunava najmanji i najveći element niza $a$ dužine $n$. 
