...
Statistički zavod Srbije istražuje kako rade obdaništa u Srbiji. Za svako obdanište poznat je spisak koji sadrži broj dece u grupi, a zatim i ocene koje je svako dete dalo o radu obdaništa. Definisati strukturu \kckod{Dete} koja sadrži polja pol (\kckod{m} ili
...
#\ulaz{m 5 4}#
#\ulaz{m 3 4}#
#\izlaz{Unesite pol i broj godina za statistiku: }\ulaz{m 3}#
...
#\ulaz{z 4 5}#
#\ulaz{m 6 3}#
#\izlaz{Unesite pol i broj godina za statistiku: }\ulaz{z 4}#
...
#\ulaz{m 3 2}#
#\ulaz{z 3 5}#
#\izlaz{Unesite pol i broj godina za statistiku: }\ulaz{h 5}#
...
\item Definisati funkciju \kckod{double obim(Tacka *poligon, int n)} koja izračunava obim poligona sa $n$ temena u zadatom nizu.
...
\item Napisati funkciju \kckod{double povrsina\_trougla(const Tacka *A, const Tacka *B, const Tacka *C)} za računanje površine trougla.

\item Napisati funkciju \kckod{double povrsina(Tacka *poligon, int n)} za računanje površine konveksnog poligona. \uputstvo{Zadatak se može rešiti podelom poligona na trouglove i korišćenjem funkcije \kckod{povrsina\_trougla}}.
\end{enumerate}
...
\item Napisati funkciju koja ispisuje polinom stepena $n$ sa koeficijentima $k_0$, $k_1$, ..., $k_n$ u obliku \kckod{$k_0 \pm
