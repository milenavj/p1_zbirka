...
%TODO ovaj zadatak je bio ponovljen - treba izabrati bolje resenje i boje test primere.
...
Napisati funkciju \kckod{int broj\_neparnih\_cifara(int x)} koja određuje broj
...
%TODO broj\_neparnih\_cifara --- izmeniti i u resenju
...
%TODO definicija Armstrongovog broja je nesto drugacija, i to treba podesiti i u zadatku i u resenju: An Armstrong number is an n-digit number that is equal to the sum of the nth powers of its digits. https://en.wikipedia.org/wiki/Narcissistic_number
Broj $a$ je Armstrongov ako je jednak sumi $n$-tih stepena svojih cifara, pri čemu je $n$ broj cifara broja $a$.
%\begin{enumerate}
%\item Napisati funkciju \kckod{int stepen(int x, int n)} koja izračunava $n$-ti stepen broja $x$.
%\item 
Napisati funkciju \kckod{int armstrong(int x)} koja vraća $1$ ako je broj Armstrongov, odnosno $0$ ako nije.
%\end{enumerate}
...
%TODO promenila sam povratni tip funkcije u unsigned --- izmeniti resenje ako treba
Napisati funkciju \kckod{long unsigned fibonaci(int n)} koja računa $n$-ti
