...
#\naslovIzlazZaGresku#
#\izlaz{Greska: datoteka ulaz.txt}#
#\izlaz{ne postoji.}#
\end{upotreba}
\end{minitest}
\linkresenje{v3_01}
...
\includecode{resenja/4_Datoteke/1/1.c}
\end{Answer}
...
\includecode{resenja/4_Datoteke/praktikumi14/1_02.c}
\end{Answer}
...
Na primer, \kckod{b} se zamenjuje sa \kckod{A},
\kckod{B} sa \kckod{a}, \kckod{a} sa \kckod{Z}, \kckod{A} sa
...
#\naslovIzlazZaGresku#
#\izlaz{Greska: datoteka podaci.txt}#
#\izlaz{ne postoji.}#
\end{upotreba}
\end{minitest}
\linkresenje{p3_id17}
...
\includecode{resenja/4_Datoteke/ciklicna_zamena.c}
\end{Answer}
...
\begin{itemize}
\item ukoliko je učitan karakter \kckod{u}, sva mala slova zamenjuje velikim
\item ukoliko je učitan karakter \kckod{l}, sva velika slova zamenjuje malim
\end{itemize}
...
\includecode{resenja/4_Datoteke/zamena.c}
\end{Answer}
...
\includecode{resenja/4_Datoteke/sveubroj.c}
\end{Answer}


\begin{Exercise}[label=p3_01] 
Napisati program koji prebrojava mala slova u datoteci $test.txt$ i dobijeni rezultat ispisuje na
standardni izlaz.
U slučaju greške, na standardni izlaz za greške ispisati odgovarajuću poruku.
...
\includecode{resenja/4_Datoteke/praktikumi14/1_01.c}
\end{Answer}
...
\includecode{resenja/4_Datoteke/naj_cifra.c}
\end{Answer}
...
\includecode{resenja/4_Datoteke/zagrade.c}
\end{Answer}
...
\begin{description}
\item{a)} Napisati C funkciju \kckod{int unesi\_skup(char s[], FILE* f)}
  kojom se unosi skup elemenata iz datoteke \kckod{F}. Skup se predstavlja kao
...
\item{b)} Napisati funkciju
  \kckod{void prebroj(char s[], int *br\_slova,int *br\_cifara)} kojom se
  određuje broj slovnih elemenata skupa (velikih ili malih slova)
  kao i broj cifara u skupu.
\item{c)} Napisati program koji koristeći prethodne funkcije prebrojava cifre i slova u datoteci 
...
#\naslovIzlazZaGresku#
#\izlaz{Greska: datoteka skup4.txt}#
#\izlaz{ne postoji.}#
\end{upotreba}
\end{minitest}
\begin{minitest}
...
%\includecode{resenja/4_Datoteke/6/6.c}
\end{Answer}
...
\begin{Exercise}[label=p3_08] 
 Napisati program koji za reč $s$ maksimalne dužine 20 karaktera koja se zadaje sa standardnog ulaza 
 u datoteku $rotacije.txt$ upisuje sve rotacije reči $s$.
 U slučaju greške, na standardni izlaz za greške ispisati odgovarajuću poruku.
...
\includecode{resenja/4_Datoteke/praktikumi14/1_08.c}
\end{Answer}
...
rotira za $k$ mesta u levo i da tako dobijenu reč upisuje u datoteku čije je ime $rotirano.txt$. 
Maksimalna dužina naziva datoteke je $20$ karaktera. Može se pretpostaviti da
...
#\izlaz{Unesite broj k: }\ulaz{7}#
#\naslovDat{tekst.dat ne postoji}#
#\naslovIzlazZaGresku#
#\izlaz{Greska: datoteka tekst.dat ne postoji.}#
\end{upotreba}
\end{miditest}
\linkresenje{p3_id18}
\end{Exercise}
\begin{Answer}[ref=p3_id18]
\includecode{resenja/4_Datoteke/rotacija.c}
\end{Answer}


\begin{Exercise}[label=p3_iv6]         
Napisati program koji iz datoteke čije se ime zadaje sa standardnog ulaza prepisuje reči na standardni 
izlaz a one reči koje sadrže prvu reč iz datoteke i podvlaku upisuje u datoteku $rez.txt$. 
Maksimalna dužina naziva
datoteke je $20$ karaktera a reči u datoteci $50$ karaktera.
U slučaju greške, na standardni izlaz za greške ispisati odgovarajuću poruku.
...
#\datoteka{Sunce\_Moje Jedan Dva Su\_nce Sve }#
#\datoteka{Sunce123\_123 suncanica.}#
#\naslovIzlaz#
#\izlaz{Sunce sija iznad grada}#
#\izlaz{Sunce\_Moje Jedan Dva Su\_nce Sve }#
#\izlaz{Sunce123\_123 suncanica.}#
#\naslovDat{rez.txt: }#
...
#\naslovIzlazZaGresku#
#\izlaz{Greska: datoteka dat4.txt ne postoji.}#
\end{upotreba}
\end{miditest}
\linkresenje{p3_iv6}
\end{Exercise}
\begin{Answer}[ref=p3_iv6]
\includecode{resenja/4_Datoteke/recipodvlaka.c}
\end{Answer}
...
Podrazumeva se da je reč palindrom ako se čita isto sa leve i desne strane bez obzira na veličinu slova. 
Maksimalna dužina reči je $200$ karaktera a maksimalan broj reči nije poznat.
U slučaju greške, na standardni izlaz za greške ispisati odgovarajuću poruku.
...
\includecode{resenja/4_Datoteke/palindrom.c}
\end{Answer}


\begin{Exercise}[label=p3_iv5]         
U datoteci čije se ime zadaje sa standardnog ulaza nalazi
se broj $n$ ($n\leq 256$), a zatim i $n$ reči dužine najviše $50$ karaktera. 
...
\includecode{resenja/4_Datoteke/ukloni_duplikate.c}
\end{Answer}
...
\includecode{resenja/4_Datoteke/zbir_ascii.c}
\end{Answer}
...
#\naslovDat{in.txt ne postoji}#
#\naslovIzlazZaGresku#
#\izlaz{Greska: datoteka in.txt ne postoji.}#
\end{upotreba}
\end{miditest}
\begin{miditest}
\begin{upotreba}{4}
...
\includecode{resenja/4_Datoteke/3/3.c}
\end{Answer}
...
\includecode{resenja/4_Datoteke/praktikumi14/1_05.c}
\end{Answer}
...
\includecode{resenja/4_Datoteke/matrica.c}
\end{Answer}
...
#\datoteka{9 3 5 2}#
#\naslovIzlazZaGresku#
#\izlaz{Greska: neispravna dimenzija.}#
\end{upotreba}
\end{minitest}
\linkresenje{p3_x6}
\end{Exercise}
...
\begin{Exercise}[label=p3_07] 
 ...
 Koristiti strukturu $Tacka$ sa poljima $x$ i $y$, kao i funkciju kojom se računa rastojanje. 
 Maksimalan broj tačaka u datoteci je $50$.
 U slučaju greške, na standardni izlaz za greške ispisati odgovarajuću poruku.
...
%\includecode{resenja/4_Datoteke/praktikumi14/1_07.c}
\end{Answer}
...
\end{verbatim}
U datoteci $vektori.txt$ nalazi se nepoznati broj vektora (najviše $200$). 
Napisati program koji učitava vektore iz ove datoteke i na standardni izlaz ispisuje 
koordinate vektora sa najvećom dužinom. Dužina vektora se izračunava po formuli:
$$|v|= \sqrt{x^2+y^2+z^2}$$
U slučaju greške, na standardni izlaz za greške ispisati odgovarajuću poruku.
...
#\datoteka{670}#
#\naslovIzlazZaGresku#
#\izlaz{Greska: neispravan broj vektora.}#
\end{upotreba}
\end{minitest}
\begin{minitest}
\begin{upotreba}{3}
...
#\datoteka{2 -1 2}#
#\naslovIzlazZaGresku#
#\izlaz{Greska: neispravan}#
#\izlaz{sadrzaj datoteke.}#
\end{upotreba}
\end{minitest}
\begin{minitest}
\begin{upotreba}{6}
...
Maksimalan broj pravougaonika je $200$.
U slučaju greške, na standardni izlaz za greške ispisati odgovarajuću poruku.
...
\includecode{resenja/4_Datoteke/pravougaonici_kvadrati.c}
\end{Answer}
...
\begin{Exercise}[label=p3_06] 
 ...
 ispisati prvog. Pretpostaviti da broj studenata neće biti veći od $100$.
 U slučaju greške, na standardni izlaz za greške ispisati odgovarajuću poruku.
...
\includecode{resenja/4_Datoteke/praktikumi14/1_06.c}
\end{Answer}
...
Maksimalan broj studenta je $100$. \uputstvo{Ime i prezime studenta se mogu pročitati pomoću
specifikatora $\%s$ a potom se za kreiranje niske $ime\_i\_prezime$ u traženom formatu može iskoristiti funkcija $strcat$.}
U slučaju greške, na standardni izlaz za greške ispisati odgovarajuću poruku.
...
#\naslovDat{ulaz.txt ne postoji}#
#\naslovIzlazZaGresku#
#\izlaz{Greska: datoteka ulaz.txt ne postoji.}#
\end{upotreba}
\end{miditest}
\begin{miditest}
\begin{upotreba}{4}
...
\includecode{resenja/4_Datoteke/5/5.c}
\end{Answer}
...
#\naslovIzlazZaGresku#
#\izlaz{Greska: datoteka dat1.txt}#
#\izlaz{ne postoji.}#
\end{upotreba}
\end{minitest}
\begin{minitest}
...
Pretpostaviti da su linije maksimalne dužine 
$80$ karaktera.
U slučaju greške, na standardni izlaz za greške ispisati odgovarajuću poruku.
...
#\naslovIzlazZaGresku#
#\izlaz{Greska: datoteka }#
#\izlaz{in.txt ne postoji.}#
\end{upotreba}
\end{minitest}
...
\includecode{resenja/4_Datoteke/2/2.c}
\end{Answer}
...
\begin{Exercise}[label=p3_03] 
 ...
 Maksimalna dužina linije je $80$ karaktera.
 U slučaju greške, na standardni izlaz za greške ispisati odgovarajuću poruku.
...
\includecode{resenja/4_Datoteke/praktikumi14/1_03.c}
\end{Answer}
...
Maksimalna dužina reči je $100$ karaktera, a maksimalna dužina linije je $500$ karaktera.
U slučaju greške, na standardni izlaz za greške ispisati odgovarajuću poruku.
...
\includecode{resenja/4_Datoteke/4/4.c}
\end{Answer}


\begin{Exercise}[label=p3_04] 
 Napisati program koji prebrojava koliko se linija datoteke $ulaz.txt$ završava niskom $s$ 
 koja se učitava sa standardnog ulaza. Može se pretpostaviti da dužina linije neće biti veća od $80$ karaktera, 
 kao i da dužina niske $s$ neće biti veća od $20$ karaktera.
 U slučaju greške, na standardni izlaz za greške ispisati odgovarajuću poruku.
...
\includecode{resenja/4_Datoteke/praktikumi14/1_04.c}
\end{Answer}
...
\begin{Exercise}[label=p3_09] 
 ...
 \textit{-v} ili \textit{-V} samo one linije koje počinju velikim slovom, 
 ako je zadata opcija \textit{-m} ili \textit{-M} samo one linije koje počinju malim slovom, 
 a ako je opcija izostavljena sve linije. Pretpostaviti da linije neće biti duže od 80 karaktera.
 U slučaju greške, na standardni izlaz za greške ispisati odgovarajuću poruku.
...
\includecode{resenja/4_Datoteke/praktikumi14/1_09.c}
\end{Answer}
...
\includecode{resenja/4_Datoteke/diff.c}
\end{Answer}
