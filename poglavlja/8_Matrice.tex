...
\includecode{resenja/2_PredstavljanjePodataka/2.4_Matrice/01.c}
\end{Answer}
...
ispisati odovarajuću poruku. \uputstvo{Euklidska norma matrice je kvadratni koren
sume kvadrata svih elemenata matrice.}

%Ovo je verovatno pogresan zadatk ostao...
...
\includecode{resenja/2_PredstavljanjePodataka/2.4_Matrice/001.c}
\end{Answer}
...
\begin{enumerate}
  \item \kckod{void ucitavanje(int mat[][MAX], int* n,
  ...
  \item \kckod{void ispis(int mat[][MAX], int n, int
    m)} kojom se ispisuju vrednosti matrice $mat$ dimenzije $m \times n$.
\end{enumerate}
...
\begin{enumerate}
  \item \kckod{int refleksivna(int a[][MAX], int
    n)} kojom se za relaciju zadatom matricom \kckod{a} dimenzije
    $n \times n$ ispituje da li je refleksivna;
  \item \kckod{int simetricna(int a[][MAX], int n)}
    kojom se za relaciju zadatom matricom \kckod{a} dimenzije
    $n \times n$ ispituje da li je
    simetrična;
  \item \kckod{int tranzitivna(int a[][MAX], int n)}
    kojom se za relaciju zadatom matricom \kckod{a} ispituje dimenzije
    $n \times n$ da li je tranzitivna;
  \item \kckod{int ekvivalencija(int a[][MAX], int
    n)} kojom se za relaciju koja je zadata metricom \kckod{a} dimenzije
...
\begin{enumerate} 
  \item Napisati funkciju \kckod{float trag(float a[][MAX], int n)}
    ...
    dijagonali matrice.
  \item Napisati funckciju \kckod{float suma\_sporedna(float a[][MAX],
    int n)} koja računa zbir elemenata na sporednoj dijagonali
    matrice.
  \item Napisati funkciju \kckod{float suma\_iznad(float a[][MAX], int n)}
    koja određuje sumu elememenata iznad glavne dijagonale.
  \item Napisati funkciju \kckod{float suma\_ispod(float a[][MAX], int n)}
...
je maksimalna dimenzija matrice $100 \times 100$.  U slučaju greške
ispisati odovarajuću poruku.
...
sporednoj) sortirani strogo rastuće. %Ispisati odgovaraju\'ce poruke.
...
$1$. Napisati program koji za unetu kvadratnu matricu proverava da 
li je ortonormirana. U slučaju greške ispisati odgovarajuću
...
proizvod učitanih matrica. Ukoliko množenje matrica nije moguće ili je
došlo do greške prilikom unosa podataka ispisati odgovarajuću
poruku. Maksimalna dimenzija obe matrice je $50 \times 50$.
...
učitanoj matrici. Maksimalna dimenzija obe matrice je $50\times 50$.
